...
CREATE TABLE comentarios(
  ...
  conteudo TEXT,
  users_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
  fotos_id INTEGER REFERENCES fotos(id) ON DELETE CASCADE,
  PRIMARY KEY(id)
);
...
INSERT INTO
  comentarios (conteudo, users_id, fotos_id)
VALUES
  (
...
-- JOIN
  -- - Utilizado para produzir algum valor, unindo duas ou mais tabelas
  -- - Utilizado para encontrar dados entre duas ou mais tabelas

-- AGGREGATION
  -- - Utilizado para calcular apenas um valor, baseado em uma ou mais colunas
  -- - Palavras como 'média', 'soma', 'máximo', 'mínimo', etc

-- Para cada comentário, pegue o conteúdo da tabela 'comentarios' e a url da tabela 'fotos'
SELECT
  conteudo,
  url
FROM
  comentarios
  JOIN fotos ON fotos.id = comentarios.fotos_id;

-- Adicionando mais uma foto, porém sem referência a nenhum users_id
 INTO
  fotos(url, users_id)
  VALUES('https://banner.jpg', NULL)

-- Quando listamos as 'url's' e 'users_id's, a foto sem referência a nenhum users_id não é mostrada
SELECT
  url,
  nome
FROM
  fotos;
  JOIN users ON users.id = fotos.users_id;

-- Para mostrar a foto sem referência a nenhum users_id, utilizamos o LEFT JOIN
SELECT
  url,
  nome
FROM
  fotos
  LEFT JOIN users ON users.id = fotos.users_id;

-- Adicionaremos mais um usuário, porém sem referência a nenhuma foto
INSERT INTO
  users(nome)
  VALUES('Nicole')

-- Para mostrar o usuário sem referência a nenhuma foto, utilizamos o RIGHT JOIN
SELECT
  url,
  nome
FROM
  fotos
  RIGHT JOIN users ON users.id = fotos.users_id;

-- Para mostrar todos os usuários e fotos, independente de terem referência ou não, utilizamos o FULL JOIN
SELECT
  url,
  nome
FROM
  fotos
  FULL JOIN users ON users.id = fotos.users_id;

-- Mostrar quais usuários comentaram nas prórpias fotos
SELECT
  url,
  conteudo
FROM
  comentarios
  JOIN users ON users.id = comentarios.users_id
  JOIN fotos ON fotos.id = comentarios.fotos_id
WHERE
  fotos.users_id = comentarios.users_id;

-- mostrar quais usuários comentaram nas próprias fotos, select url, conteudo e nome
SELECT
  url,
  conteudo,
  nome

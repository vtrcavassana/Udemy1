...
CREATE TABLE fotos(
  id SERIAL PRIMARY KEY,
  url TEXT,
  users_id SERIAL REFERENCES users(id) ON DELETE CASCADE
);
CREATE TABLE comentarios(
  id SERIAL PRIMARY KEY,
  conteudo TEXT,
  fotos_id SERIAL REFERENCES fotos(id) ON DELETE CASCADE,
  users_id SERIAL REFERENCES users(id) ON DELETE CASCADE
);
...
-- GROUP BY
-- - Encontra um valor único para cada grupo de valores
-- - É usado para agrupar valores semelhantes

-- Quero saber os users_id que possuem comentários agrupados por users_id
SELECT users_id FROM comments GROUP BY users_id;

-- AGGREGATE FUNCTIONS
-- - Funções que retornam um valor único
-- - São usadas para calcular valores agregados
...
-- Qual o maior id de comentário de cada usuário? (Aqui, o GROUP BY é necessário, pois se não tiver, o resultado será o maior id de todos os comentários)
SELECT
  users_id,
  MAX(id)
FROM
...
-- Quantidade de comentários em cada foto, usando o GROUP BY e AGGREGATE
SELECT
  photos_id,
  COUNT(*) -- * significa que quero todos os campos
FROM
  comentarios
GROUP BY
  fotos_id

-- Quantidade de comentários em cada foto, onde fotos_id é menor que 3 e tenha mais que 2 comentários
SELECT
  fotos_id,
  COUNT(*) -- * significa que quero todos os campos
FROM
...
-- Encontrar os usuários que comentaram nas fotos 1 e 2 e que tenham mais de 2 comentários nessas fotos
SELECT
  users_id,
  COUNT(*) -- * significa que quero todos os campos
FROM
...
-- Encontrar os usuários que comentaram nas primeras 50 fotos e que tenham mais de 20 comentários nessas fotos
SELECT
  users_id,
  COUNT(*) -- * significa que quero todos os campos
FROM
